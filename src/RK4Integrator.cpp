 ...
 * Calculates forces for the springs.
*/

#include "RK4Integrator.h"

State RK4Integrator::evaluate(const State &_initial, float _t	)
{

  State output;
  output.m_position=_initial.m_velocity;
  output.m_velocity=motionFunction(_initial);
  return output;
}
//----------------------------------------------------------------------------------------------------------------------
State RK4Integrator::evaluate( const State &_initial, float _t,float _dt, const State &_d	)
{
  ...
  output.m_velocity = motionFunction(state);
  return output;
}
//----------------------------------------------------------------------------------------------------------------------
void RK4Integrator::integrate(float _t, float _dt	)
{
